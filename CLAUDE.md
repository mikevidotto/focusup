...
- **Event.Important:** a plain `bool` on `Event`/`OccurrenceView` for emphasis in compact views (e.g. a red dot vs. a neutral dot on the widget's week/month grid — a birthday vs. a routine garbage-day reminder). There's still no add/edit-event UI at all (`AddEvent` is Go-binding-only, unreached from any form), so nothing can actually set `Important` to `true` yet outside a hand-edited `calendar.json` or a one-off script — worth flagging for whoever builds that form next.
- **Per-occurrence completion tracking:** `Event.Completions []Completion` (`event.go`) — same shape and matching rule as `Exception` (matched to an occurrence by calendar date against its raw/un-rescheduled date, resolved in `Occurrences()` alongside exceptions), but for "did I actually do this" rather than "does this occurrence's schedule change." Surfaces as `Occurrence.Done`/`OccurrenceView.done`. Unlike `Important`, this *does* have a real UI path already: `calendar.Service.ToggleCompletion` / `app.go`'s `ToggleEventCompletion` binding, wired to a clickable checkbox in `CalendarPage.svelte`'s detail panel and month-grid event chips, and `CalendarWidget.svelte`'s Today agenda (a mouse-click action, not gated by any keyboard-focus state). The widget's week/month dot indicator is 3-way: green when every occurrence that day is done, red if any remaining one is `important`, grey otherwise.
- **"Take out the garbage"** is a real weekly (Thursday 7am, reminder Wed 7pm) event seeded into the dev machine's actual `calendar.json` via a one-off `go run` script (not committed) — expect to see it in this project's own dashboard/Calendar screenshots.
